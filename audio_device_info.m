...
CFArrayRef CreateInputDeviceArray()
{
    AudioObjectPropertyAddress propertyAddress = {
        ...
        kAudioObjectPropertyElementMain  // Updated to kAudioObjectPropertyElementMain
    };
...
int main() {
    ...
    if (devices) {
        ...
        for (CFIndex i = 0; i < count; i++) {
            ...
            CFStringRef deviceUID = CFDictionaryGetValue(device, CFSTR("deviceUID"));
            CFStringRef deviceName = CFDictionaryGetValue(device, CFSTR("deviceName"));
            const char *uid = CFStringGetCStringPtr(deviceUID, kCFStringEncodingUTF8);
            const char *name = CFStringGetCStringPtr(deviceName, kCFStringEncodingUTF8);
            printf("Device UID: %s, Device Name: %s\n", uid ? uid : "unknown", name ? name : "unknown");
        }
        CFRelease(devices);
    }
    return 0;
